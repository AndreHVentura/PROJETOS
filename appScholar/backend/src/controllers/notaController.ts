...
export const cadastrarNota = async (req: Request, res: Response) => {
  try {
    const { alunoMatricula, disciplinaId, nota1, nota2, nota3, nota4, nota5 } = req.body;
    ...
    const aluno = await Aluno.findOne({ where: { matricula: alunoMatricula } }) as AlunoInstance | null;
    if (!aluno) {
    ...
    const notas = [nota1, nota2, nota3, nota4, nota5].filter(nota => nota !== undefined && nota !== null);
...
export const atualizarNota = async (req: Request, res: Response) => {
  try {
    ...
    const notas = [nota1, nota2, nota3, nota4, nota5].filter(n => n !== undefined && n !== null);
